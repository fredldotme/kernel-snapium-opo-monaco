...
extern int data_wakeup_hook_init(void);
extern int data_wakeup_hook_fini(void);
...
static int __init linkpower_main_init(void)
{
	...
	ret = data_wakeup_hook_init();
	if (ret < 0) {
		printk("[linkpower_main] module failed to init data_wakeup_hook.\n");
		linkpower_netlink_fini();
		sk_pid_hook_fini();
		return ret;
	}
