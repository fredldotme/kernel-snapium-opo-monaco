...
EXPORT_SYMBOL_GPL(haptic_hv_i2c_reads);
...
EXPORT_SYMBOL_GPL(haptic_hv_i2c_writes);
...
EXPORT_SYMBOL_GPL(haptic_hv_i2c_write_bits);
...
static int aw_stop_notify_call(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	if(((event == SWITCH_PRE_ACTION) && (pdata[1] == 0)) 		 /*motor switch to mcu*/
		...
		if(get_switch_status_by_id(SWITCH_MT))
			...
			aw_err("%s,missing mt switch!",__func__);
	}
