...
static void read_hardware_register_host_wake(void)
{
...
dhdsdio_suspend(void *context)
{
	...
	if (SLPAUTO_ENAB(bus) && (!ret) && (bus->dhd->up) && (bus->dhd->op_mode != DHD_FLAG_HOSTAP_MODE)) {
		if (wait_event_timeout(bus->bus_sleep, bus->sleeping, wait_time) == 0) {
			if (!bus->sleeping) {
				DHD_ERROR(("%s %d %d %d %d %d %d %d\n", __FUNCTION__, bus->readframes, bus->reqbussleep, bus->dpc_sched, bus->rxskip, pktq_n_pkts_tot(&bus->txq), bus->tx_seq, bus->rx_seq));
				ret = 1;
			}
		}
	}
