	...
	const wl_event_msg_t *e, void* data)
{
	...
	case WLC_E_DISASSOC:
		...
		WL_MSG_RLMT(dev->name, &e->addr, ETHER_ADDR_LEN,
			"disconnected with "OPLUSMACDBG", event %d, reason %d\n",
			OPLUSMAC2STRDBG((u8 *)wrqu.addr.sa_data), event_type, reason);
		break;
	case WLC_E_DEAUTH_IND:
	case WLC_E_DISASSOC_IND:
		cmd = SIOCGIWAP;
		WL_MSG(dev->name, "disconnected with "OPLUSMACDBG", event %d, reason %d\n",
			OPLUSMAC2STRDBG((u8 *)wrqu.addr.sa_data), event_type, reason);
	...
	case WLC_E_LINK:
		...
		if (!(flags & WLC_EVENT_MSG_LINK)) {
			WL_MSG(dev->name, "Link Down with "OPLUSMACDBG", reason=%d\n",
				OPLUSMAC2STRDBG((u8 *)wrqu.addr.sa_data), reason);
			bzero(wrqu.addr.sa_data, ETHER_ADDR_LEN);
			bzero(&extra, ETHER_ADDR_LEN);
#ifdef WL_EXT_IAPSTA
			wl_ext_in4way_sync_wext(dev, STA_NO_BTC_IN4WAY,
			...
			WL_MSG(dev->name, "Link UP with "OPLUSMACDBG"\n",
				OPLUSMAC2STRDBG((u8 *)wrqu.addr.sa_data));
		}
		break;
