...
long long temp_raw;
#ifdef MATCH_OPLUS_PLATFORM
int show_wakeup_pkt = 1;
module_param(show_wakeup_pkt, int, 0644);
#endif
...
MODULE_IMPORT_NS(OPLUS_WLAN_EXPORT_ONLY);
...
#else
			if (show_wakeup_pkt) {
				print_hex_dump(KERN_ERR, "dhd:", DUMP_PREFIX_OFFSET, 16, 1, eth, MIN(len, 64), 1);
			}
#endif
			update_wake_pkt_info(skb);
#ifdef MATCH_OPLUS_PLATFORM
			DHD_ERROR(("update_wake_pkt_info %llx\n", temp_raw));
#endif
#ifdef CONFIG_IRQ_HISTORY
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)) && !defined(MATCH_OPLUS_PLATFORM)
...
dhd_register_if(dhd_pub_t *dhdp, int ifidx, bool need_rtnl_lock)
{
	...
	printf("Register interface [%s]  MAC: "OPLUSMACDBG"\n\n", net->name,
#if defined(CUSTOMER_HW4_DEBUG)
		OPLUSMAC2STRDBG(dhd->pub.mac.octet));
#else
		OPLUSMAC2STRDBG(net->dev_addr));
#endif /* CUSTOMER_HW4_DEBUG */
...
#ifdef MATCH_OPLUS_PLATFORM
extern int dhd_oplus_custom_init(void);
#endif /* MATCH_OPLUS_PLATFORM */
...
#ifdef MATCH_OPLUS_PLATFORM
	err = dhd_oplus_custom_init();
	if (err) {
		pr_err("dhd_oplus_custom_init %d\n", err);
	}
#endif /* MATCH_OPLUS_PLATFORM */
