	...
	struct cfg80211_scan_request *request)
{
	...
	if (params_v2) {
		...
		params_v2->active_time = htod32(CUSTOM_PRODUCT_ACTIVE_TIME);
		params_v2->passive_time = htod32(CUSTOM_PRODUCT_PASSIVE_TIME);
#else
...
#ifdef MATCH_OPLUS_PLATFORM
		params->active_time = htod32(CUSTOM_PRODUCT_ACTIVE_TIME);
		params->passive_time = htod32(CUSTOM_PRODUCT_PASSIVE_TIME);
#else
...
#endif /* LINUX_VER > 4.11 */
{
	...
	mutex_unlock(&cfg->scan_sync);
#ifdef MATCH_OPLUS_PLATFORM
	mutex_unlock(&cfg->oplus_pno_sync);
#endif
	return 0;
}
