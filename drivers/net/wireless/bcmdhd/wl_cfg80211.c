...
wl_conn_debug_info(struct bcm_cfg80211 *cfg, struct net_device *dev, wlcfg_assoc_info_t *info)
{
	...
	if (info->reassoc) {
		...
		WL_MSG(dev->name, "Reconnecting with " OPLUSMACDBG " ssid \"%s\", len (%d), "
			"channel=%s-%d(chan_cnt=%d), sec=%s, rssi=%d => %d\n",
			OPLUSMAC2STRDBG((u8*)(&info->bssid)), info->ssid, info->ssid_len,
		...
		WL_MSG(dev->name, "Connecting with " OPLUSMACDBG " ssid \"%s\", len (%d), "
			...
			OPLUSMAC2STRDBG((u8*)(&info->bssid)), OPLUS_SSID_MASK(info->ssid), info->ssid_len,
			CHSPEC2BANDSTR(chanspec), wf_chspec_ctlchan(chanspec), chan_cnt,
	...
	u16 reason_code)
{
	...
	WL_MSG(dev->name, "Reason %d, act %d, bssid "OPLUSMACDBG"\n", reason_code, act, OPLUSMAC2STRDBG(curbssid));
	...
	u8 key_idx, const u8 *mac_addr, struct key_params *params)
{
	...
	WL_MSG(dev->name, "key index (%d) for "OPLUSMACDBG"\n", key_idx, OPLUSMAC2STRDBG(mac_addr));
	if ((bssidx = wl_get_bssidx_by_wdev(cfg, dev->ieee80211_ptr)) < 0) {
		WL_ERR(("Find p2p index from wdev(%p) failed\n", dev->ieee80211_ptr));
		return BCME_ERROR;
	}
...
wl_handle_link_down(struct bcm_cfg80211 *cfg, wl_assoc_status_t *as)
{
	...
	BCM_REFERENCE(dhdp);
#endif /* BCMDONGLEHOST */
	WL_MSG(ndev->name, "Link down: %s(%d), reason %d from "OPLUSMACDBG"\n",
		bcmevent_get_name(as->event_type), event, reason, OPLUSMAC2STRDBG(as->addr));
	...
	if (memcmp(as->curbssid, as->addr, ETHER_ADDR_LEN) != 0) {
		WL_ERR(("BSSID of event is not the connected BSSID"
			"(ignore it) cur: " OPLUSMACDBG
			" event: " OPLUSMACDBG"\n",
			OPLUSMAC2STRDBG(as->curbssid),
			OPLUSMAC2STRDBG((const u8*)(&as->addr))));
		return 0;
	}
	...
	const wl_event_msg_t *event, void *data)
{
	...
	if ((status == WLC_SUP_KEYED || status == WLC_SUP_KEYXCHANGE_WAIT_G1) &&
		...
		((ANDROID_VERSION >= 13) && (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 78)))
